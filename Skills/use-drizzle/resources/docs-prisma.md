...
**Source URL:** https://orm.drizzle.team/docs/prisma
...
[ ESLint Plugin ](/docs/eslint-plugin)[ drizzle-zod ](/docs/zod)[ drizzle-typebox ](/docs/typebox)[ drizzle-valibot ](/docs/valibot)[ drizzle-arktype ](/docs/arktype)[ drizzle-graphql ](/docs/graphql)
...
In order to use Drizzle query builder, you need references to Drizzle tables. You can import them from the output path that you specified in the generator config.
    ...
    await prisma.$drizzle.insert().into(User).values({ email: '[[email protected]](/cdn-cgi/l/email-protection)', name: 'Søren' });
